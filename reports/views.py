...
def monthly_attendance_summary(request):
    ...
    records = AttendanceRecord.objects.filter(date__range=[start_date, end_date], employee__restaurant__in=restaurants)
    ...
    for emp in employees.values():
        emp['compliance'] = round((emp['days_present'] / emp['days_scheduled']) * 100) if emp['days_scheduled'] else 0
...
def monthly_attendance_detail(request):
    ...
    monthly_attendance = AttendanceRecord.objects.filter(date__range=[start_date, end_date], employee__restaurant__in=restaurants)
...
def individual_attendance_report(request):
    ...
    employees_with_attendance = AttendanceRecord.objects.filter(date__range=[start_date, end_date], employee__restaurant__in=restaurants)
    employee_ids = employees_with_attendance.values_list('employee', flat=True).distinct()
    employees = Employee.objects.filter(id__in=employee_ids, restaurant__in=restaurants).order_by('first_name')
    ...
    if selected_employee_id:
        try:
            selected_employee = Employee.objects.get(id=selected_employee_id, restaurant__in=restaurants)
