...
class UserSetupForm(forms.ModelForm):
    ...
    restaurants = forms.ModelMultipleChoiceField(
        ...
        widget=forms.SelectMultiple(
            attrs={"class": "fieldstyle w-3/4", "id": "id_restaurants"}
        ),
    )
    ...
    def __init__(self, *args, **kwargs):
        ...
        if self.request_user and self.request_user.role == "gm":
            self.fields["role"].choices = [
                ("manager", "Manager")
            ]  # GM can only create managers
            self.fields["role"].widget.attrs[
                "readonly"
            ] = True  # Make it readonly instead of disabled
            ...
            if self.request_user.restaurant:
                self.fields["restaurant"].queryset = Restaurant.objects.filter(
                    id=self.request_user.restaurant.id
                )
                self.fields["restaurant"].initial = self.request_user.restaurant
                self.fields["restaurant"].widget.attrs[
                    "readonly"
                ] = True  # Read-only, not disabled
            else:
                self.fields["restaurant"].queryset = Restaurant.objects.none()
                self.fields["restaurant"].help_text = (
                    "You must have an assigned restaurant to create a manager."
                )
...
class PasswordResetForm(forms.Form):
    new_password = forms.CharField(
        widget=forms.PasswordInput(attrs={"class": "block text-center mx-auto"})
    )
