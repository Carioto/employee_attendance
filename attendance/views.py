...
def attendance_entry(request):
    ...
    if request.user.role in ["gm", "manager"]:
        ...
        employees = Employee.objects.filter(
            restaurant__in=request.user.restaurants.all()
        ).order_by("first_name")
    ...
    if request.method == "POST":
        ...
        if formset.is_valid():
            ...
            for index, form in enumerate(formset):
                emp_id = form.cleaned_data.get("employee_id") or request.POST.get(
                    f"form-{index}-employee_id"
                )
                ...
                if emp_id:
                    ...
                    logger.debug(
                        f"Saving attendance: {employee.first_name} {employee.last_name}, Date={selected_date}, Code={code}, Comment={comment}"
                    )
                    ...
                    if code != "OFF":  # Skip saving if "Not Scheduled"
                        ...
                        logger.debug(
                            f"Skipped saving attendance for {employee.first_name} {employee.last_name} - Marked as OFF"
                        )
